...
import { GET as connections } from '../src/app/api/projects/[id]/connections/route'
...
test('Companies: legacy adoption, isolation, roles, revocation and last owner', { skip: process.env.COMPANY_INTEGRATION !== '1' }, async () => {
  ...
  try {
    ...
    for (const [handler, method] of [[readEnv, 'GET'], [saveEnv, 'POST'], [deploy, 'POST'], [pause, 'POST'], [connections, 'GET'], [deleteInstance, 'DELETE'], [domain, 'PUT'], [database, 'POST']] as const) {
